...
router.post('/course-upload',async(req,res,next) => {
  try {
    ...
    if (
      ...
      for (let i = 0; i < 4; i++) {
        ...
        excelData.forEach(async (obj) => {
          ...
          const doesExist = await Course.findOne({ courseCode: obj.courseCode });
          if (!doesExist) {
               const course = new Course(obj);
               await course.save();
          }else {
            await Course.updateOne({ courseCode: obj.courseCode } , {
    ...
    console.log("Your file is uploaded successfully and users are registered successfully");
  } else {
